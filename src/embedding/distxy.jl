...
function distxy!(X::GraphEmbedding, nits::Integer = 0, verbose::Bool = true)
    D = dist_matrix(X.G)
    x0 = vector_out(X)
    ...
    if verbose
        msg = "$(res.f_calls) function calls\t" * "score = $(score(x1))"
        @info msg
    end
...
function distxy!(X::GraphEmbedding, tol::Float64 = 0.001, verb::Bool = true)
    x0 = Inf
    x1 = 0.0
    # if verb
    #     tic()
